...
class ModeSelectScene:
        ...
        self.modes = [
            {
                ...
                "name": "PARTIDO RÁPIDO",
                "icon": "[FAST]",
                "desc": "Un partido amistoso completo con equipos de ligas globales.",
                "color": (0, 200, 150),
            },
            {
                "id": "tournament",
                "name": "TORNEO",
                "icon": "[COPA]",
                "desc": "Eliminatorias directas hacia la gloria mundial.",
                "color": (255, 215, 0),
            },
            {
                "id": "free",
                "name": "JUEGO LIBRE",
                "icon": "[LIBRE]",
                "desc": "Practica sin límites de tiempo.",
                "color": (100, 150, 255),
            },
            {
                "id": "viewer",
                "name": "PLANTELES",
                "icon": "[PLANT]",
                "desc": "Consulta equipos, planteles y estadísticas.",
                "color": (180, 120, 255),
            },
            {
                "id": "career",
                "name": "CARRERA",
                "icon": "[MGR]",
                "desc": "Gestiona tu propio club como entrenador o forja tu leyenda como jugador.",
                "color": (255, 100, 80),
            },
        ]
    ...
    def _proceed(self):
        ...
        if opt == "friendly":
            ...
            self.context["mode"] = "free"
            self.context["tournament_type"] = None
            from scenes.league_select import LeagueSelectScene
            self.manager.set_scene(LeagueSelectScene, context=self.context)
        elif opt == "viewer":
            self.context["mode"] = "viewer"
            self.context["tournament_type"] = None
            from scenes.league_select import LeagueSelectScene
            self.manager.set_scene(LeagueSelectScene, context=self.context)
        elif opt == "career":
            self.context["mode"] = "career"
            from scenes.career_setup import CareerSetupScene
            self.manager.set_scene(CareerSetupScene, context=self.context)
    ...
    def draw(self, surface):
        ...
        card_h = 350
        gap = 20
        total_w = card_w * len(self.modes) + gap * (len(self.modes) - 1)
